# 分析`vue-cli@2.9.6` 搭建的`webpack`项目工程
### 前言
...
// 后面有些process模块用到的，这里列举一下。
// 更多查看这篇阮一峰的文章 http://javascript.ruanyifeng.com/nodejs/process.html
...
if (config.build.productionGzip) {
  ...
  webpackConfig.plugins.push(
    new CompressionWebpackPlugin({
      ...
      asset: '[path].gz[query]',
      // algorithm： 可以是 function(buf, callback) 或者字符串。对于字符串来说依照 zlib 的算法(或者 zopfli 的算法)。默认值是 "gzip"。algorithm: 'gzip',
...
至此，就算相对完整的分析完了`Vue-cli`(版本`v2.9.6`)搭建的`webpack`项目工程。希望对大家有所帮助。<br>
...
2、后续有余力，可能会继续分析新版的`@vue/cli`构建的`webpack`项目工程。
